...
def test_players_persisted(db_session):
...
def test_career_stats_persisted(db_session):
...
def test_year_validation(db_session):
...
def test_uses_cached_html(mock_download_url, db_session):
...
def test_http_error_propagates(mock_download_url, mock_save_html, mock_exists):
